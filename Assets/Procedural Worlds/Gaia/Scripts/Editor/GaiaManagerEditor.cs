    ...
    public class GaiaManagerEditor : EditorWindow, IPWEditor
    {
        ...
        public void OnEnable()
        {
            ...
            var creationTabs = new Tab[] {
                ...
                new Tab ("BuildAndDeploy", BuildAndDeployTab),
            };
        ...
        private void DrawSceneListElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            if (m_buildConfig.m_sceneBuildEntries[index].m_masterScene.name == EditorSceneManager.GetActiveScene().name)
            {
        ...
        private void DoMaintenance(bool forceMaintenanceRun = false)
        {
            ...
            if (!m_settings.m_shaderReimportRestartRequired && ((!System.IO.Directory.Exists(GaiaUtils.GetAssetPath("Dev Utilities")) && File.Exists(GaiaDirectories.GetSettingsDirectory() + "\\" + GaiaConstants.maintenanceTokenFilename))) || forceMaintenanceRun)
            {
                ...
                else
                {
                    ...
                    {
                        if (EditorUtility.DisplayDialog("Gaia Installer", "Gaia is installing, this will take around 5 minutes.\r\n\r\nIn the meantime you can register with Canopy to receive FREE stamp packs for the Gaia stamper.\r\n\r\nCanopy is our new community site for all Procedural Worlds Tools with support forums, knowledge base library, tutorials, and much more. Care to take a look?", "Yes, visit Canopy website!", "No thanks"))
                        {
                            string version = "Gaia";
#if GAIA_PRO_PRESENT
                            version += "Pro";
                            ...
                            Application.OpenURL("https://canopy.procedural-worlds.com/?utm_source=" + version + "&utm_medium=UnityEditor&utm_campaign=Canopy");
                        }
                    }
